...
kriging <- function(data, data.coord, krig.coord, cov.mod = "whitmat",
  ...
  if (dist.dim != dist.dim.krig) {
    stop(paste("The conditioning locations live in R^", dist.dim,
      " but you want predictions in R^", dist.dim.krig, "!",
      sep = ""
    ))
  }
  ...
  if (grid) {
    ...
    for (i in 1:nrow(krig.coord)) {
      new.krig.coord <- rbind(new.krig.coord, cbind(
        krig.coord[, 1],
        krig.coord[i, 2]
      ))
    }
  ...
  icovMat <- covariance(
    nugget = 0, sill = sill, range = range, smooth = smooth,
  ...
  return(list(
    coord = krig.coord, krig.est = krig, grid = grid,
    weights = weights
  ))
}
