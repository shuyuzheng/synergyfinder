...
PlotSynergy <- function(data, type = "2D", save.file = FALSE, pair.index = NULL,
  ...
  for (block in blocks) {
    ...
    pp <- data.frame(
      x = rep(colnames(mat.tmp), each = nrow(mat.tmp)),
    ...
    if (type == "3D") {
      ...
      if (!is.null(col.range)) {
        xaxis <- list(
          at = seq(1, ncol(mat.tmp), by = len + 1),
      ...
      if (!is.null(row.range)) {
        yaxis <- list(
          at = seq(1, nrow(mat.tmp), by = len + 1),
      ...
      fig <- lattice::wireframe(t(mat.tmp),
      ...
      graphics::axis(3,
        tick = FALSE, at = lattice::do.breaks(
          c(
            start.point,
            end.point
          ),
      ...
      graphics::par(
        mar = c(6.1, 6.1, 1.1, 2.1), cex.axis = axis.lab.cex,
      ...
      graphics::plot.window(
        asp = NA, xlim = range(x.2D), ylim = range(y.2D),
        "", xaxs = "i", yaxs = "i"
      )
      ...
      graphics::axis(
        side = 1, at = seq(0, 1, by = 1 / (length(xconc) - 1)),
        labels = xconc
      )
      graphics::axis(
        side = 2, at = seq(0, 1, by = 1 / (length(yconc) - 1)),
        labels = yconc
      )
      ...
      syn.3d.plot <- lattice::wireframe(t(mat.tmp),
        scales = par1, drape = TRUE,
        colorkey = list(
          space = "top", width = 0.5,
          labels = list(cex = legend.lab.cex)
...
.ExtendedScores <- function(scores.mat, len) {
  ...
  krig.coord <- cbind(
    rep(extended.row.idx, each = ext.col.len),
    rep(extended.col.idx, times = ext.row.len)
  )
  extended.scores <- kriging(
    data = c(scores.mat),
    data.coord = cbind(
      rep(seq_len(nr), nc),
