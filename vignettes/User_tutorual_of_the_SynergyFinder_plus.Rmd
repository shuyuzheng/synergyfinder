...
Combinatorial therapies have been recently proposed for improving anticancer treatment efficacy. To facilitate the drug combination discovery, high-throughput drug combination screening has the advantage of assaying a large collection of chemical compounds, generating dynamic dose-response profiles that allow us to quantify the degree of drug-drug interactions at an unprecedented level. *SynergyFinder* R package is a software tool to analyze pre-clinical drug combination data sets. It provides efficient implementations for 

1. all the popular synergy scoring models, including HSA, Loewe, Bliss, and ZIP to quantify the degree of drug synergy;
...
4. synergy barometer to harmonize multiple synergy scoring methods to provide a consensus metric of synergy;
5. evaluation of drug combination synergy and sensitivity simultaneously to provide an unbiased interpretation of the clinical potential.

To facilitate the use of the R package for the drug discovery community, we also provide a web server at http://synergyfinderplus.org/ that provides a user-friendly interface to enable a more flexible and versatile analysis of drug combination data. In the web application, in addition to the functions in R package, we provide the annotation of drugs and cell lines that are tested in an experiment, including their chemical information, targets and signaling network information.

# 1. Install and load the SynergyFinder

**Note:** It is recommend to install *Bioconductor >= 3.13* and *SynergyFinder >= 3.0.1* to access the new features described in this vignette. 
...
*SynergyFinder* provides an example input data (`mathews_screening_data`) in the package, which is extracted from a drug combination screening study for the treatment of diffuse large B-cell lymphoma (DLBCL) [1]. The example input data contains two representative drug combinations (ibrutinib & ispinesib and ibrutinib & canertinib) for which the %viability of a cell line TMD8 was assayed using a 6 by 6 dose matrix design. To load the example data, we use the command:
...
* `method`: The reference model for synergy score calculation. Parameter `Emin` and `Emax` are used to set the correspoding parameters in the 4 parameter log-logistic model which will be fitted to calculate "ZIP" and "Loewe" synergy scores;
* `correct_baseline`: Adjust base line of response matrix to make it closer to 0 by calling function `CorrectBaseLine`. We provide three options:
...
### 2. 2D contour plot (`Plot2DrugContour`)
...
The calculation of RI and relative IC50 for monotherapies in higher-order drug combination is same as that is used in 2-drug combination data. Please check section 4.2 for more details.
